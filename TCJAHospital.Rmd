...
kable(
  excluded_states,
  caption = "(Table 2) States Excluded from DID Analysis",
...
ggplot(data_all_years_plotFTEpayrolllog, 
       aes(x = Year, 
           y = avg_LOGFTEpayroll, 
           color = Type.of.Control, 
           group = Type.of.Control,
           linetype = Type.of.Control)) +  
  geom_line(linewidth = 1.2) +   
  geom_point(size = 2) + 
  geom_vline(xintercept = 2018, linetype = "dashed", color = "red", linewidth = 1) + 
  labs(x = "Year", 
       y = "Log of FTE Employees on Payroll", 
       color = "Hospital Type",
       linetype = "Hospital Type",       
       title = "(Figure 1) Parallel Trends Check for log(FTE Employees)") +
  theme_minimal() + 
  ylim(5, 6.5)
...
ggplot(data_all_years_plotsalaryLOG, 
       aes(x = Year, 
           y = avg_salaryLOG, 
           color = Type.of.Control, 
           group = Type.of.Control,
           linetype = Type.of.Control)) +  # map line type to group
  geom_line(linewidth = 1.2) +   
  geom_point(size = 3) + 
  geom_vline(xintercept = 2018, linetype = "dashed", color = "red", linewidth = 1) + 
  labs(x = "Year", 
       y = "Log of Total Salary from Worksheet A", 
       color = "Hospital Type",
       linetype = "Hospital Type",   # legend for line types
       title = "(Figure 2) Parallel Trends Check for log(Total Salary)") +
  theme_minimal() + 
  coord_cartesian(ylim = c(16, 17.5))  # safer than ylim
```
...
```{r}
variable_names <- c(
  "FTE...Employees.on.Payroll"       = "FTE Employees",
  "Total.Salaries.From.Worksheet.A"  = "Total Salaries",
  "Number.of.Beds"                   = "Number of Beds",
  "Total.Discharges.Title.XVIII"     = "Total Discharges",
...
numeric_summary %>%
  kable(
    caption = "(Table 3) Descriptive Statistics of Numeric Hospital Variables",
    align = "lccccc",
...
numeric_summary_by_group <- numeric_summary_by_group %>%
  mutate(Variable = recode(Variable, !!!variable_names))

# Format table
numeric_summary_by_group %>%
  kable(
    caption = "(Table 4) Descriptive Statistics by Ownership Type",
    align = "lccccc",
...
data_diff2019 <- data_all_years_filtered %>%
  ...
  summarize(
    ...
    for_profit = first(for_profit),
     d_beds = diff(log(Number.of.Beds)),
    d_discharges = diff(log(Total.Discharges.Title.XVIII)),
    d_urban = diff(urban)
  )
model_diff2019 <- lm(d_LOGFTE ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2019)
model_diff2019sal <- lm(d_TotalSal ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2019)
...
model_diff2020 <- lm(d_LOGFTE ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2020)
model_diff2020sal <- lm(d_TotalSal ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2020)
...
model_diff2021 <- lm(d_LOGFTE ~ for_profit  + d_beds  + d_discharges + d_urban, data = data_diff2021)
model_diff2021sal <- lm(d_TotalSal ~ for_profit  + d_beds  + d_discharges + d_urban, data = data_diff2021)
...
model_diff2022 <- lm(d_LOGFTE ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2022)
model_diff2022sal <- lm(d_TotalSal ~ for_profit + d_beds  + d_discharges + d_urban,  data = data_diff2022)
...
modelsummary(
  ...
  coef_map = c(
  `(Intercept)`      = "Intercept",
  `post:for_profit` = "Post*For Profit",
...
modelsummary(
  ...
  `post`             = "Post",
  `beds`             = "Number of Beds",
  `log(Total.Discharges.Title.XVIII)`             = "Total Discharges",
  `log(Total.Costs)`             = "Total Costs",
  `urban`             = "urban"
...
```
```{r}
names(coef(did_emp20172019salF))
```
...
modelsummary(
  ...
  stars = TRUE,
  gof_omit = "AIC|BIC|Log.Lik|F|Adj",  
  title = "Table 9: First Differencing Regression log(FTE Employees)",
  output = "markdown",
  coef_map = c(
  `(Intercept)`      = "Intercept",
  `for_profit` = "For Profit",
  `post`             = "Post",
  `d_beds`             = "Number of Beds",
  `d_income`             = "Income",
  `d_discharges`             = "Discharge",
  `urban`             = "urban"
...
modelsummary(
  ...
  stars = TRUE,
  gof_omit = "AIC|BIC|Log.Lik|F|Adj",  
  title = "Table 10: First Differencing Regression log(Total Salary)",
  output = "markdown"
)
```
...
```{r}
models <- list(
  "FTE Employee 2019" = model_diff2019,
  "FTE Employee 2020" = model_diff2020,
  "FTE Employee 2021" = model_diff2021,
  "FTE Employee 2022" = model_diff2022,
  
  "Total Salaries 2019" = model_diff2019sal,
  "Total Salaries 2020" = model_diff2020sal,
  "Total Salaries 2021" = model_diff2021sal,
  "Total Salaries 2022" = model_diff2022sal
